...
func testRecipeToManyReports(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, reportDBTypes, false, reportColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, reportDBTypes, false, reportColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Reports().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RecipeSlice{&a}
	if err = a.L.LoadReports(ctx, tx, false, (*[]*Recipe)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Reports); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Reports = nil
	if err = a.L.LoadReports(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Reports); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRecipeToManyAddOpReports(t *testing.T) {
	var err error
	...
	foreigners := []*Report{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, reportDBTypes, false, strmangle.SetComplement(reportPrimaryKeyColumns, reportColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Report{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddReports(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Reports[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Reports[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Reports().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testRecipeToOneCategoryUsingCategory(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCategory(ctx, tx, false, (*[]*Recipe)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRecipeToOneSetOpCategoryUsingCategory(t *testing.T) {
	var err error
...
var (
	recipeDBTypes = map[string]string{`ID`: `char`, `Title`: `varchar`, `ThumbnailURL`: `varchar`, `Recipe`: `text`, `CategoryID`: `int`, `Ingredient`: `json`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_             = bytes.MinRead
)
