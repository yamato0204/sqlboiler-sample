...
func testCategoryToManyRecipes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.Recipes().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadRecipes(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Recipes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Recipes = nil
	if err = a.L.LoadRecipes(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Recipes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCategoryToManyAddOpRecipes(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.R.Recipes[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Recipes[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Recipes().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
