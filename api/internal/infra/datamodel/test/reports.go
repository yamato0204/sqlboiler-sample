...
func testReportsExists(t *testing.T) {
	...
	e, err := ReportExists(ctx, tx, o.ID)
	if err != nil {
...
func testReportsFind(t *testing.T) {
	...
	reportFound, err := FindReport(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testReportToOneRecipeUsingRecipe(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, reportDBTypes, false, reportColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Report struct: %s", err)
	}
	...
	AddRecipeHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Recipe) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := ReportSlice{&local}
	if err = local.L.LoadRecipe(ctx, tx, false, (*[]*Report)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testReportToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ReportSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Report)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
		t.Error("struct should have been eager loaded")
...
func testReportToOneSetOpRecipeUsingRecipe(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, reportDBTypes, false, strmangle.SetComplement(reportPrimaryKeyColumns, reportColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
...
func testReportToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	reportDBTypes = map[string]string{`ID`: `varchar`, `Comment`: `text`, `ThumbnailURL`: `varchar`, `UserID`: `varchar`, `RecipeID`: `char`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_             = bytes.MinRead
)
