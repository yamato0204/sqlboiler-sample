...
func testUserToManyReports(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadReports(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpReports(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `varchar`, `Name`: `varchar`, `Email`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_           = bytes.MinRead
)
